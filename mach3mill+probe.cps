...
function onCyclePoint(x, y, z) {
  ...
  if (isFirstCyclePoint()) {
    ...
    // Probing routines - Added by Daniel
    case "probing-x":
      break;
    case "probing-y":
      break;
    case "probing-z":
      //Probe Z
      writeBlock(gMotionModal.format(52), "Z0")             //RESET OFFSET
      writeBlock(mFormat.format(1));                        //Optional STOP
      writeBlock(feedOutput.format(F));                     //Set Feed
...
function onCycleEnd() {
  // Start of code added by Daniel
  if (isProbeOperation()) {
    // add probe code here
  } else {
    // End of code added by Daniel
    if (cycleSubprogramIsActive) {
      subprogramEnd();
      cycleSubprogramIsActive = false;
    }
